...
> [!NOTE]
> The storage account referenced by `deploymentScriptSettings.storageAccountId` must have Shared Key access (`allowSharedKeyAccess`) enabled internally, even if you disable its public network access entirely - deployment scripts always mount the file share using the storage account key, regardless of network configuration. If your tenant also uses a policy that requires `allowSharedKeyAccess` to be disabled, request a scoped policy exemption for this storage account; there's no supported way to run deployment scripts against a storage account with Shared Key access disabled.
>
...
>
> When `deploymentScriptSettings.subnetId` is set, in addition to the storage account key, Azure also requires the `Storage File Data Privileged Contributor` role on the script-runner identity for the storage account (this template grants it automatically). See [Access a private virtual network from a Bicep deployment script](https://learn.microsoft.com/en-us/azure/azure-resource-manager/bicep/deployment-script-vnet-private-endpoint).
