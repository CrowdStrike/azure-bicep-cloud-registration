...
| `deploymentScriptSettings.storageAccountId`                                  | no       | Resource ID of an existing storage account for deployment scripts to use instead of the auto-provisioned one. Required if the target tenant's Azure Policy disallows public network access on auto-created storage accounts. Must be in the same subscription as `csInfraSubscriptionId` - deployment scripts only support an existing storage account from their own subscription. Note: due to a current Microsoft platform limitation (tracked at [Azure/bicep#16604](https://github.com/Azure/bicep/issues/16604)), deployment scripts cannot authenticate to a storage account with Shared Key access (`allowSharedKeyAccess`) disabled on the storage account itself - see `deploymentScriptSettings.storageAccountKey` and `.subnetId` below for how the key is supplied. |
...
> [!NOTE]
> The storage account referenced by `deploymentScriptSettings.storageAccountId` must have Shared Key access (`allowSharedKeyAccess`) enabled internally, **regardless of whether `deploymentScriptSettings.subnetId` is set**. This is confirmed by testing: with `allowSharedKeyAccess: false`, deployment fails with `DeploymentScriptACIProvisioningTimeout` even when using `deploymentScriptSettings.subnetId` and the RBAC role granted automatically below - the underlying Azure Container Instance never starts provisioning. Enabling `Storage File Data Privileged Contributor` RBAC lets you omit the key *value* from `deploymentScriptSettings.storageAccountKey` when `subnetId` is set, but the storage account's Shared Key *capability* must still be enabled. If your tenant enforces a policy requiring `allowSharedKeyAccess` to be disabled, there's currently no supported way to run deployment scripts against that storage account; request a scoped policy exemption instead.
>
> The storage account's own firewall rules (`networkAcls`) must **not** restrict access (leave `defaultAction` at `Allow`, or omit `networkAcls` entirely) - Microsoft's deployment script service doesn't support firewall-restricted storage accounts, independent of `publicNetworkAccess`. Set `publicNetworkAccess: 'Disabled'` on the storage account instead; that alone fully blocks public network access without triggering this limitation.
>
> When `deploymentScriptSettings.subnetId` is set, the deployment script runs inside a virtual network and can mount the storage account using the script-runner identity's `Storage File Data Privileged Contributor` role (granted automatically), so `deploymentScriptSettings.storageAccountKey` can be omitted - but `allowSharedKeyAccess` must still be `true` on the storage account (see above). See [Access a private virtual network from a Bicep deployment script](https://learn.microsoft.com/en-us/azure/azure-resource-manager/bicep/deployment-script-vnet-private-endpoint).

### Deployment fails with DeploymentScriptStorageAccountWithServiceEndpointEnabled

If the storage account referenced by `deploymentScriptSettings.storageAccountId` has `networkAcls.defaultAction` set to `Deny` (a storage account firewall rule), deployment fails with:

```
DeploymentScriptStorageAccountWithServiceEndpointEnabled: Storage account '<name>' has firewall settings enabled which are not supported for deployment scripts.
```

`Microsoft.Resources/deploymentScripts` doesn't support an existing storage account with firewall rules enabled, regardless of `networkAcls.bypass` or `virtualNetworkRules` settings. Set `networkAcls.defaultAction` to `Allow` (or omit `networkAcls` entirely) on the storage account. This doesn't reopen public access - `publicNetworkAccess: 'Disabled'` on the storage account is the control that actually blocks the public network path, and it's unaffected by `networkAcls`.

If the error persists after fixing `networkAcls`, confirm `deploymentScriptSettings.subnetId` is set and that the `Storage File Data Privileged Contributor` role assignment (see the note above) actually deployed - Azure returns this same error code when that role assignment is missing, even with a correctly configured firewall.
